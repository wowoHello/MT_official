...
description: Reviews.razor / ReviewService / ReviewModels 的實作現況摘要，含狀態流轉、三審制度、迴避規則、效能優化歷史。最後對照程式碼全面校正於 2026-06-01。
type: project
---

## 現況摘要（最後更新：2026-06-01 結案前核對）

### 三檔行數（wc -l 實際量測）
- `ReviewService.cs`：1704 行
...
實作模式：先 SELECT 取 existingIds（1 次），再 foreach UPDATE（N 次，N=子題數，通常 2-5 個）。三種題型分支：
...
- **UpsertFinalSubQuestionsAsync**：SELECT existingIds + foreach UPDATE，子題數通常 2-5 個（現況已落地，無後續排程）

**Why:** 2026-06-01 結案前對照三個完整檔案核對，修正行數（RS=1702→1704）、罐頭訊息（8 則舊描述→CWT/LCT 各 6 則）、DecisionBar 已解鎖配置（移除錯誤的「儲存草稿」按鈕描述）。
**How to apply:** 改動三審決策流程、退回計次、Status 流轉前，先以本記憶確認當前實作細節，再評估改動範圍。
