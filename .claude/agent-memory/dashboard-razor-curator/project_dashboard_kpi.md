...
description: 4 張 KPI 卡片 + 2 張 ApexCharts 圖表 + 逾期待辦 + LOG 分頁的資料來源、Status 碼對應、Service 查詢結構、JS Interop 模式（2026-05-19 卡片 3 母/子題拆分版）
type: project
---

Dashboard 頁面完整實作現況（以 2026-05-19 程式碼為準，覆蓋舊版描述）。
...
- `DashboardService.cs`：~1000 行（含多個私有方法與內部私有型別）
- `DashboardModels.cs`：~320 行（全部 DTO + Enum）
...
- 主數字：審題階段（PhaseCode 3/5/7）**分母/子題拆分顯示**：「母題 X/Y · 子題 X/Y」
  ...
  - **完成判定：DecidedAt IS NOT NULL**（非 Comment 非空；草稿只 UPDATE Comment 不寫 DecidedAt）
...
7. `GetReviewProgressAsync` → **(ReviewPhaseLabel, masterReviewed, masterTotal, subReviewed, subTotal)** — 5-tuple，母/子題各自計數，完成判定改為 `DecidedAt IS NOT NULL`
...
**DashboardKpiDto 新增欄位（2026-05-19）**：
- `ReviewedSubCount`：子題已審數（DecidedAt IS NOT NULL + SubQuestionId IS NOT NULL）
- `ReviewTotalSubCount`：子題應審總數（SubQuestionId IS NOT NULL 的全部分配）
- `ReviewedCount` 語意精化：母題已審（DecidedAt IS NOT NULL + SubQuestionId IS NULL）
- `ReviewTotalCount` 語意精化：母題應審總數（SubQuestionId IS NULL 的全部分配）

**BuildUrgentItemsAsync 審題落後分支（4-c）的判定同步更新**：
- `DecidedAt IS NOT NULL` 判已審；`DecidedAt IS NULL` 視為尚未完成審題
- 過去版本用 `Comment IS NOT NULL`，現已全面替換
