...
description: 4 張 KPI 卡片 + 2 張 ApexCharts 圖表 + 逾期待辦 + LOG 分頁的資料來源、Status 碼對應、Service 查詢結構、JS Interop 模式（2026-05-08 全面校正版）
type: project
---

Dashboard 頁面完整實作現況（以 2026-05-08 程式碼為準，覆蓋舊版描述）。
...
**注意：舊記憶描述是靜態 Top 5 列表，實際已升級為分頁 + Filter Chips + Toggle。**

### 狀態變數
- `logPage: AuditLogPage?`（null = 尚未載入）
- `isLoadingLogs: bool`
- `includeGlobalLogs: bool`（localStorage key: "cwt-dashboard-log-include-global"）
- `activeLogFilter: LogTypeFilter`（初始 All）
- `logPrefLoaded: bool`（避免 firstRender 重複讀 localStorage）
- `LogPageSize = 50`（每頁筆數）

### Filter Chips（LogTypeFilter 7 種）
- All / Login / Members / Project / Announcement / Question / Review
- 排列：全部 > 登入 > 人員 > 專案 > 公告 > 試題 > 審題

### Toggle「含全站事件」
- iOS-style Toggle（Tailwind 純 CSS 實作，無 JS 元件）
- 狀態持久化至 localStorage（值為 "1"/"0"）
- 開啟時含登入/登出、教師管理、全站公告等跨梯次事件（ProjectId IS NULL）

### 載入邏輯
- `LoadLogsAsync(reset: true)`：首次 / 切換梯次 / 切換 Toggle / 切換 Filter 時觸發
- `LoadLogsAsync(reset: false)`：「載入更早 N 筆」按鈕追加（合併 Logs List）
- 首次讀 localStorage（OnAfterRenderAsync firstRender）完成後才呼叫首次 LoadLogs

### LOG 列表 UI
- 每列：圓角圖示(w-7 h-7) + 描述 + 相對時間（tabular-nums）
- max-h-[290px] overflow-y-auto（約 5 筆捲動）
- 登入/登出（Action 3/4）：「{UserName} 登入/登出系統」（無 TargetType 標籤）
- 其他：「{UserName} 動詞了 類型「名稱」」（truncate + title tooltip）
- 相對時間：<60s=剛剛 / <60m=N分鐘前 / <24h=N小時前 / <7d=N天前 / else=MM/dd HH:mm
- 「載入更多」按鈕（HasMore=true 時顯示，顯示剩餘筆數）

### Action 碼
- 0=建立(emerald+sage) / 1=修改(blue+morandi) / 2=刪除(rose+terracotta) / 3=登入(indigo) / 4=登出(stone)

### TargetType 碼
- 0=使用者 / 1=角色 / 2=梯次 / 3=試題 / 4=公告 / 5=教師 / 6=審題
...
- `Components/Pages/Dashboard.razor`：4 KPI 卡片 + 2 圖表 + 逾期待辦（含教師 Modal） + LOG 分頁（含 Filter Chips + Toggle）+ 所有 Helper 方法
