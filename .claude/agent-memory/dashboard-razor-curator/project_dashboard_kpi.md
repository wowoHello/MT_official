...
## 三檔案規模（2026-05-22 實測）
- `Dashboard.razor`：~1415 行（UI + @code，大型頁面）
- `DashboardService.cs`：~1819 行（含多個私有方法與內部私有型別）
- `DashboardModels.cs`：~420 行（全部 DTO + Enum）
...
- 主數字：修題階段（PhaseCode 4/6/8）顯示合計 XX/OO 題（RevisedCount/RevisionTotalCount），下方再細拆母/子兩行
  - **Model 欄位拆分（2026-05-22 更新）**：DashboardKpiDto 已拆為 4 個細分欄位：
    - `RevisedMasterCount` / `RevisionMasterTotal`：母題（SubQuestionId IS NULL）已修/待修
    - `RevisedSubCount` / `RevisionSubTotal`：子題（SubQuestionId IS NOT NULL）已修/待修
    - `RevisedCount`（computed property）= RevisedMasterCount + RevisedSubCount
    - `RevisionTotalCount`（computed property）= RevisionMasterTotal + RevisionSubTotal
  - **UI 顯示**：主數字顯示合計（text-4xl），下方 flex flex-col 顯示「母題 X / Y」/ 「子題 A / B」（僅在各自有值時顯示）
...
- `Components/Pages/Dashboard.razor`：4 KPI 卡片 + 2 圖表 + 逾期待辦（教師 Modal）+ LOG 分頁；包含修補 E（兩張圖表 JS interop 並行，chartTasks List<Task> WhenAll）
...
## 圖表 2 空狀態判定（2026-05-22 更新）
圖表 2 的空狀態判定邏輯已從「所有桶都 0 視為無資料」放寬為「列表非空即渲染」：
```razor
else if (!kpi.StatusByType.Any())
{
    <EmptyState ... />
}
else
...
```
原因：CWT/LCT 都透過 SQL JOIN 確保桶列表非空（CWT 6 桶 / LCT 5 桶），新建梯次設定目標後立即看到棒結構（空棒 = 0），與「題型缺口達成率」圖表呈現一致。
